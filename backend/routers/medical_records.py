...
def list_medical_records(
    ...
    return query.order_by(models.MedicalRecord.record_date.desc()).offset(skip).limit(limit).all()
...
def create_medical_record(record: schemas.MedicalRecordCreate, db: Session = Depends(get_db)):
    if not db.query(models.Patient).filter(models.Patient.patient_id == record.patient_id).first():
        raise HTTPException(status_code=404, detail="Patient not found")
    if not db.query(models.Doctor).filter(models.Doctor.doctor_id == record.doctor_id).first():
...
def list_prescriptions(record_id: int, db: Session = Depends(get_db)):
    if not db.query(models.MedicalRecord).filter(models.MedicalRecord.record_id == record_id).first():
        raise HTTPException(status_code=404, detail="Medical record not found")
    return (
...
@router.post("/{record_id}/prescriptions", response_model=schemas.Prescription, status_code=201)
def add_prescription(
    record_id: int, prescription: schemas.PrescriptionCreate, db: Session = Depends(get_db)
):
...
def delete_prescription(prescription_id: int, db: Session = Depends(get_db)):
    ...
    if medicine:
        medicine.stock_quantity += presc.quantity
